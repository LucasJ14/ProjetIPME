var game = new Phaser.Game(1080, 256, Phaser.AUTO, 'game', { preload: preload, create: create, update: update, render:render});
...
function create() {
  game.physics.startSystem(Phaser.Physics.ARCADE);
    game.stage.backgroundColor = '#020820';

  map = game.add.tilemap('objects');
  map.addTilesetImage('items', 'tiles');
  layer = map.createLayer('Capa de Patrones 1');
  layer.resizeWorld(); //scrolling
  layer.wrap = true;
  map.setCollisionBetween(14, 16); //murs
  map.setCollisionBetween(21, 22); //haut des tuyaux
  map.setCollisionBetween(27, 28); //bas des tuyaux
  map.setCollisionByIndex(10);
  map.setCollisionByIndex(13);
  map.setCollisionByIndex(17);//drapeau
  map.setCollisionByIndex(40);//sol
...
function render() {
  game.debug.bodyInfo(mario.sprite, -200, -200);
}